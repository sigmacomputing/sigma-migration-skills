...
# --- Task 4: <Container> card wrappers (styled layouts) ---
...
legacy_alias_layout = %Q(<GridContainer elementId="legacy" type="grid" gridColumn="1 / 25" gridRow="1 / 7" gridTemplateColumns="repeat(24, 1fr)">
  <LayoutElement elementId="a" gridColumn="1 / 25" gridRow="1 / 7"/>
</GridContainer>)
check('lint parser accepts legacy layout aliases for historical artifacts') do
  CompositionLint.check(legacy_alias_layout).empty?
end

legacy_spec = {
  'pages' => [{ 'id' => 'p', 'name' => 'Overview' }],
  'elements' => [{ 'id' => 'a', 'kind' => 'text', 'body' => 'Revenue' }],
  'layout' => %Q(<Page id="p">#{legacy_alias_layout}</Page>)
}
check('layout-quality lint accepts legacy aliases for historical artifacts') do
  LayoutLint.lint(legacy_spec).empty?
end
