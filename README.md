...
`docs/agent-entry.md`. For example, Cortex Code:
...
- **No converter binary to install** — each skill bundles its data-model converter (`convert_*_to_sigma`) and runs it **locally by default** (no network, no data egress). A hosted converter MCP is available as an **optional, opt-in fallback** (set `SIGMA_MCP_CONVERTER_URL`) that sends the source spec off-machine.
- **Optional:** Sigma MCP for interactive read/query during parity — not required for the REST pipeline (`docs/agent-entry.md`).
