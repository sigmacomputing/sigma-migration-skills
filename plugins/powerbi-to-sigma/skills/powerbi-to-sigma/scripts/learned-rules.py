#!/usr/bin/env python3
"""learned-rules — load + apply gap-scout translation rules (Qlik & Power BI).

Rules live in the customer's HOME (`~/.powerbi-to-sigma/learned-rules.yaml` or
`~/.powerbi-to-sigma/learned-rules.yaml`), NOT the skill repo — so `git pull` of the
skill never clobbers what a customer's scout has discovered, and wins persist across
every app/report they migrate. Override the dir with the matching `*_HOME` env var.

The conversion build step imports this and calls `apply()` on each source expression
before falling back to the converter / a WARN line.

    from learned_rules import load, apply
    rules = load(home="~/.powerbi-to-sigma")
    sigma_formula, hint = apply(rules, qlik_expr)   # (None, None) if no rule matches
"""
import os, re, yaml
...
def home_dir(home=None, env=None):
    ...
    return os.path.expanduser("~/.powerbi-to-sigma")

def load(home=None, env="POWERBI_TO_SIGMA_HOME"):
