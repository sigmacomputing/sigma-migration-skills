...
- `dm-spec.json` — the DM to POST (`/v2/dataModels/spec`): a single SQL element
  over `DEMO_DB.DEMO.ORDER_FACT` exposing `NET_REVENUE` / `ORDER_CHANNEL` / `SHIP_METHOD`.
...
## LIVE (against a warehouse, e.g. DEMO_DB.DEMO)
...
**LIVE-verified 2026-07-13** against DEMO_DB.DEMO (fresh workbook + DM): workbook
