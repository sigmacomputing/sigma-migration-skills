...
Then do the manual formula pass and final shape checks per `reference/workflows/validate.md`. The validator rejects bare self-references such as `name: Region` + `formula: "[Region]"`, and list/segmented controls that have `filters` but no value-list `source` (the API accepts that control shape, but the picker renders empty). Fix everything reported before continuing.
...
Also run `./scripts/validate-spec.sh` against the GET readback. This catches a
list/segmented value-list `source` that is absent from the live document after
the write; a surviving `filters` target does not populate the picker.
