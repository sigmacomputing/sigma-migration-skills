#!/usr/bin/env python3
"""cognos-dm-signature.py — build a DM-reuse signature from the Cognos converter output.
...
pbi-dm-signature.py. Emits the signature shape find-or-pick-dm expects:
  ...
  { "tableau_workbook": "<module name>",          # label only
...
Usage: python3 scripts/cognos-dm-signature.py --dm-spec dm.json --out dm-signature.json
"""
...
def main():
    ...
    sig = {"tableau_workbook": spec.get("name") or "Cognos data module",
           "warehouse_tables": sorted(set(tables)),
           "referenced_columns": sorted(set(cols)),
           "measures": measures}
    json.dump(sig, open(a.out, "w"), indent=2)
    print(f"[cognos-dm-signature] {len(elements)} element(s): {len(sig['warehouse_tables'])} table(s), "
