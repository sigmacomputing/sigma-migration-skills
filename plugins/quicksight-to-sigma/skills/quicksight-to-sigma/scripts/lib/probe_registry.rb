...
#     after the POST can then never orphan an id untraceably — a crash-recovery
#     sweep deletes leftovers from this registry (tableau-to-sigma ships
#     scripts/sweep-run-artifacts.rb; the line shapes below are the shared
#     format it reads).
...
#     record so a sweep (and the planned E7.4 gate) can tell cleaned from
#     outstanding.
#   - LOCAL state only: <workdir>/probe-artifacts.jsonl when the caller has a
#     workdir, else ~/.quicksight-to-sigma/probe-artifacts.jsonl (dev probes).
...
module ProbeRegistry
  ...
  def home_dir
    ENV['QUICKSIGHT_TO_SIGMA_HOME'] || File.expand_path('~/.quicksight-to-sigma')
  end
  ...
  # (already gone); a 'failed' delete leaves the entry outstanding so a
  # sweep retries it.
