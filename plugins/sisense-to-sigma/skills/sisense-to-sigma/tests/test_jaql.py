...
def check_flag(label, jaql, warning_id=None):
    global passed, failed
    try:
        J.classify(jaql); failed += 1; print(f"  FAIL {label}: expected Unsupported, got a translation")
    except J.Unsupported as exc:
        if warning_id is None or exc.warning_id == warning_id:
            passed += 1
        else:
            failed += 1
            print(f"  FAIL {label}: warning_id={exc.warning_id!r}, want {warning_id!r}")
...
# --- FLAG: every catalog row with no clean Sigma equivalent ---
for fn, row in J.FLAG_FUNC.items():
    check_flag(f"flag {fn}",
               {"formula": f"{fn}([m])",
                "context": {"[m]": {"dim": "[Commerce.Revenue]", "agg": "sum"}}},
               row["warning_id"])

# Unknown callables fail closed through the catalog fallback contract.
check_flag("flag unknown function",
           {"formula": "MYSTERY([m])",
            "context": {"[m]": {"dim": "[Commerce.Revenue]", "agg": "sum"}}},
           J.UNKNOWN_FUNC["warning_id"])

# --- FLAG: filtered/scoped measures (no clean 1:1) ---
check_flag("flag filtered measure",
           {"formula": "[m]", "context": {"[m]": {"dim": "[Commerce.Revenue]", "agg": "sum", "filter": {"members": ["New"]}}}},
           "SISENSE-JAQL-FILTERED-MEASURE")
check_flag("flag unresolvable ctx",
           {"formula": "[m]", "context": {"[m]": {"foo": 1}}},
